...
class SimpleMovingAverage(Indicator):
    ...
    def __init__(self, window:int=2):
        self._window = window
        self._nans = np.array([np.NaN for _ in range(self._window - 1)]) 
    ...
    def np_run(self, data:np.ndarray):
        ...
        np_sma = (cumsum_vec[self._window:] - cumsum_vec[:-self._window]) / self._window
        
        return np.concatenate([self._nans, np_sma]) #adding NaN values to get pandas functionality